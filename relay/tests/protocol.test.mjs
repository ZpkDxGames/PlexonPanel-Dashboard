...
test("v2 claims and client-invented scopes are never upgraded implicitly", async () => {
 const base={version:3,protocolVersion:3,role:"Observer",scopes:["telemetry.view"],audience:"plexonpanel-relay",serverId:"4e34679e-30b6-4f68-a50e-a10dfdb2724d",deviceId:"92435470-1a9d-4dfc-a0e9-0b66f229a504",generation:1,issuedAt:1700000000,expiresAt:1700000600};
 for(const changed of [{version:1,protocolVersion:2},{scopes:["shell.execute"]},{role:""},{generation:-1}]) {
  const token=await signDashboardAccess({...base,...changed},"x".repeat(64));
  await assert.rejects(()=>verifyDashboardAccess(token,"x".repeat(64),1700000010));
 }
});

test("verifies the shared Java/TypeScript canonical vector",async()=>{
 const {readFile}=await import("node:fs/promises");const fixture=JSON.parse(await readFile(new URL("../fixtures/v3-envelope.json",import.meta.url),"utf8"));
 const decoded=decodeEnvelope(JSON.stringify(fixture.envelope));assert.deepEqual(decoded.body,fixture.body);assert.equal(await verifyEnvelope(decoded.envelope,await importAgentPublicKey(fixture.publicKey)),true);
});
