...
export async function pairingLookupId(
  code: string,
  pepper: string,
): Promise<string> {
...
export async function opaqueClientKey(
  address: string,
  pepper: string,
): Promise<string> {
...
export async function signDashboardAccess(
...
export async function verifyDashboardAccess(
  ...
  if (token.length > 8192) throw new Error("Dashboard token too large");
