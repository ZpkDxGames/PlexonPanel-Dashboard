...
interface RateRow { window_started_at: number; attempts: number }
...
export class CoordinationStore {
  ...
  private migrate(): void {
    ...
    `);
    const row = this.db.prepare("SELECT value FROM relay_meta WHERE key='schema_version'").get() as unknown as
      | TextRow
      | undefined;
    if (!row) {
  ...
  loadRoom(serverId: string): StoredRoom {
    const row = this.db.prepare("SELECT state_json AS value FROM server_rooms WHERE server_id=?").get(serverId) as unknown as
      | TextRow
      | undefined;
  ...
  registerPairing(registration: PairingRegistration): "ok" | "collision" {
    return this.transaction(() => {
      ...
      `).run(
        registration.lookupId, registration.serverId, registration.requestId,
        registration.challengeId, registration.fingerprint, registration.expiresAt,
      );
      return "ok";
    });
  }
  ...
  resolvePairingByLookup(lookupId: string): PairingRegistration | null {
    ...
    `).get(lookupId) as unknown as PairingRow | undefined;
    if (!row) return null;
    if (row.expires_at <= Date.now()) { this.deletePairing(lookupId); return null; }
    return {
  ...
  consumePairingRateLimit(clientKey: string, now = Date.now()): boolean {
    return this.transaction(() => {
      ...
      ).get(clientKey) as unknown as RateRow | undefined;
  ...
  counts(): { rooms: number; pairings: number; rateLimits: number } {
    const count = (table: string) => Number(
      (this.db.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get() as unknown as CountRow).count,
    );
