...
const RETIRED_COORDINATION_TYPES = new Set([
  "backup.coordination",
...
export function assertFreshEnvelope(
  ...
  if (!Number.isFinite(timestamp) || Math.abs(now - timestamp) > 30_000) {
    throw new Error(
      "Protocol message timestamp is outside the allowed clock skew",
    );
  }
}

export async function importAgentPublicKey(
...
export async function signEnvelope(
  ...
  if (RETIRED_COORDINATION_TYPES.has(type))
    throw new Error("Retired coordination message type");
...
export async function publicKeyFingerprint(
  ...
  const digest = new Uint8Array(
    await crypto.subtle.digest("SHA-256", fromBase64(publicKeyBase64)),
  );
...
function validateEnvelope(envelope: ProtocolEnvelope): void {
  ...
  if (RETIRED_COORDINATION_TYPES.has(envelope.type))
    throw new Error("Retired coordination message type");
