import { MAX_ENVELOPE_BYTES, assertFreshEnvelope, decodeEnvelope, importAgentPublicKey, publicKeyFingerprint, signEnvelope, verifyChallengeProof, verifyEnvelope, } from "./protocol.js";
...
const METADATA_KEY = "room-metadata", DIRECTORY_NAME = "pairing-directory-v3", INTERNAL_HEADER = "X-Plexon-Internal";
    ...
    async fetch(request, env) {
        ...
        if (request.method === "POST" && url.pathname === "/v1/pairings/claim") {
            ...
            if (!/^[\p{L}\p{N} ._()-]{1,64}$/u.test(name))
                return json({ ok: false, error: "Choose a short device name." }, 400, cors);
            const directory = env.PAIRING_DIRECTORY.get(env.PAIRING_DIRECTORY.idFromName(DIRECTORY_NAME));
            const clientKey = await opaqueClientKey(request.headers.get("CF-Connecting-IP") ?? "unknown", env.PAIRING_CODE_PEPPER);
            const resolved = await directory.fetch(internalRequest("/resolve", env, { code, clientKey }));
            if (!resolved.ok)
                return json({
                    ok: false,
                    error: resolved.status === 429
                        ? "Too many pairing attempts. Try again in 15 minutes."
                        : "Invalid or expired pairing code.",
                }, resolved.status === 429 ? 429 : 403, cors);
            const registration = (await resolved.json());
            const room = env.SERVER_ROOMS.get(env.SERVER_ROOMS.idFromName(registration.serverId));
            const claim = await room.fetch(internalRequest("/claim", env, {
            ...
            return json({
                ...
                accessToken: await signDashboardAccess(access, env.ACCESS_TOKEN_SECRET),
        ...
        if (request.method === "GET" && url.pathname === "/v1/dashboard/session") {
            ...
            if (!authorization.startsWith("Bearer "))
                return json({ ok: false, error: "Dashboard credential is required" }, 401, cors);
            let access;
            try {
            ...
            const result = await room.fetch(internalRequest("/validate", env, { access }));
            if (!result.ok)
                return json({
                    ok: false,
        ...
        if (request.method === "GET" && url.pathname === "/v1/agent") {
            ...
            if (request.headers.get("X-PlexonPanel-Protocol") !== "3")
                return json({
                    ok: false,
                    error: "Protocol 3 required. Upgrade the agent and re-pair browsers.",
                    code: "PROTOCOL_MISMATCH",
                }, 426);
            const serverId = url.searchParams.get("serverId") ?? "", kind = url.searchParams.get("agentKind") ?? "PAPER";
        ...
        if (request.method === "GET" && url.pathname === "/v1/dashboard") {
            ...
            if (!protocols.includes("plexonpanel-v3") || !authentication)
                return json({ ok: false, error: "Protocol 3 authentication required" }, 401);
            let access;
            try {
                access = await verifyDashboardAccess(authentication.slice(5), env.ACCESS_TOKEN_SECRET);
            }
            catch {
                return json({ ok: false, error: "Device credential invalid or expired" }, 401);
            }
...
export class PairingDirectory {
    ...
    async fetch(request) {
        if (!authorizedInternal(request, this.env))
        ...
        if (url.pathname === "/resolve") {
            ...
            const code = normalizePairingCode(body.code);
            const lookupId = await pairingLookupId(code, this.env.PAIRING_CODE_PEPPER);
            const registration = await this.state.storage.get(`code:${lookupId}`);
    ...
    async alarm() {
        ...
        for (const [key, value] of entries) {
            const expiresAt = key.startsWith("code:")
                ? value.expiresAt
                : value.windowStartedAt + 15 * 60_000;
            if (expiresAt <= now)
                expired.push(key);
            else
...
export class ServerRoom {
    ...
    async fetch(request) {
        if (!authorizedInternal(request, this.env))
            return json({ ok: false }, 403);
        const url = new URL(request.url), metadata = await this.metadata();
        if (url.pathname === "/validate") {
            const body = (await request.json());
            return json({ ok: currentAccess(metadata, body.access) }, currentAccess(metadata, body.access) ? 200 : 403);
        }
        if (url.pathname === "/claim") {
            const body = (await request.json()), pending = metadata.currentPairing;
            if (!pending ||
            ...
            try {
                ...
                const deviceId = requiredUuid(body.deviceId, "deviceId"), name = requiredText(body.name, "name", 64);
        ...
        const role = request.headers.get("X-Plexon-Role"), serverId = request.headers.get("X-Plexon-Server-Id") ?? "";
        ...
        let attachment;
        if (role === "dashboard") {
            const access = JSON.parse(request.headers.get("X-Plexon-Access") ?? "null");
    ...
    async webSocketMessage(socket, message) {
        ...
        const task = this.messages
            ...
            try {
                if (a.role === "agent")
                    await this.agentMessage(socket, a, message);
                else
                    await this.dashboardMessage(socket, a, message);
            }
            catch (error) {
                if (a.role === "agent") {
                    const code = protocolRejectionCode(error, a.authenticated === true);
                    this.recordDiagnostic(code ?? "INTERNAL_POST_AUTH_FAILURE", a, messageTypeHint(message));
                    if (code || !a.authenticated)
                        this.safeClose(socket, 4008, `Protocol message rejected: ${code ?? "INVALID_ENVELOPE"}`);
                }
                else {
                    this.safeDashboardSend(socket, {
                        type: "relay.error",
    ...
    async agentMessage(socket, a, text) {
        ...
        if (envelope.type === "agent.hello") {
            ...
            const key = requiredText(body.publicKey, "publicKey", 512), fingerprint = requiredText(body.publicKeyFingerprint, "fingerprint", 64);
            if (!(await verifyEnvelope(envelope, await importAgentPublicKey(key))) ||
            ...
            if (body._sequence !== 1)
                throw new Error("Invalid initial sequence");
            a.sequence = 1;
        ...
        if (body._session !== a.sessionNonce ||
        ...
        if (envelope.type === "agent.challenge_response") {
            if (a.authenticated ||
                ...
                !(await verifyChallengeProof(a.challenge, body.proof, await importAgentPublicKey(a.publicKey))))
                throw new Error("Challenge rejected");
            if (!a.candidate)
                throw new Error("Missing candidate identity");
            if (a.kind === "PAPER")
                m.identity = a.candidate;
            else
                m.hostIdentity = a.candidate;
            ...
            for (const other of this.agents(a.kind)) {
                if (other !== socket) {
                    const previous = other.deserializeAttachment();
                    previous.authenticated = false;
                    other.serializeAttachment(previous);
                    other.close(4002, "Agent reconnected");
                }
            }
            if (!(await this.sendGatewaySafely(socket, a.serverId, "gateway.authenticated", {
                protocolVersion: 3,
                paired: m.paired,
            ...
            if (a.kind === "HOST")
                await this.sendGatewaySafely(socket, a.serverId, "paper.connection", { connected: this.agents("PAPER").length > 0 }, a);
            return;
        }
        if (!a.authenticated)
        ...
        if (envelope.type === "pairing.accepted" ||
            envelope.type === "pairing.denied") {
            if (a.kind !== "PAPER")
                throw new Error("Only Paper approves grants");
            const id = requiredUuid(body.requestId, "requestId"), waiter = this.pairingWaiters.get(id);
            if (!waiter)
                return;
        ...
        if (envelope.type === "access.sync") {
            ...
            if (Number(body.generation) < m.generation)
                return;
            if (Number(body.generation) === m.generation &&
        ...
        if (envelope.type === "maintenance.coordination") {
            ...
            if (body.deviceId !== undefined)
                requiredUuid(body.deviceId, "deviceId");
            ...
            if (operation === "notice") {
                ...
                if (/[\0\r\n]/.test(message))
                    throw new Error("Invalid maintenance message");
                if (body.title !== undefined && body.title !== null) {
                    const title = requiredText(body.title, "title", 160);
                    if (/[\0\r\n]/.test(title))
                        throw new Error("Invalid maintenance title");
                }
            }
        ...
        if (envelope.type === "maintenance.coordination.result") {
            if (a.kind !== "PAPER")
                throw new Error("Only Paper reports maintenance coordination");
            requiredUuid(body.requestId, "requestId");
            const operation = requiredText(body.operation, "operation", 24);
            if (operation !== "notice" && operation !== "flush")
                throw new Error("Invalid maintenance operation");
        ...
        if (envelope.type === "action.result") {
            const requestId = requiredUuid(body.requestId, "requestId"), pending = this.pending.get(requestId);
            if (!pending ||
                pending.kind !== a.kind ||
        ...
        if (!AGENT_EVENTS.has(envelope.type))
            return;
        if (a.kind === "HOST" &&
            !new Set(["telemetry.system", "service.status", "backup.progress"]).has(envelope.type))
            throw new Error("Event not allowed for host");
        if (envelope.type === "players.presence")
            validatePresenceEvent(body);
        for (const peer of this.state.getWebSockets("dashboard")) {
            const d = peer.deserializeAttachment();
            if (!currentAccess(m, d.access))
                continue;
            const identity = a.kind === "HOST" ? m.hostIdentity : m.identity;
            const filtered = filterEvent(envelope.type, body, d.access.scopes, identity?.capabilities ?? {});
            if (filtered)
                this.safeDashboardSend(peer, {
                    type: "server.event",
                    serverId: a.serverId,
    ...
    async dashboardMessage(socket, a, text) {
        ...
        try {
            const action = requiredText(message.action, "action", 64), scope = ACTION_SCOPES[action], parameters = message.parameters;
            ...
            const rate = ((transfer ? a.transferRate : a.rate) ?? []).filter((t) => t > Date.now() - 10000);
            if (rate.length >= (transfer ? 160 : 20))
                throw new Error("RATE_LIMITED");
            if (transfer)
                a.transferRate = [...rate, Date.now()];
            else
                a.rate = [...rate, Date.now()];
            socket.serializeAttachment(a);
    ...
    async registerPairing(socket, a, m, body) {
        const requestId = requiredUuid(body.requestId, "requestId"), code = normalizePairingCode(body.code), expiresAt = Date.parse(requiredText(body.expiresAt, "expiresAt", 64));
        ...
        if (m.currentPairing)
            await this.directory().fetch(internalRequest("/delete", this.env, {
        ...
        const response = await this.directory().fetch(internalRequest("/register", this.env, { ...registration, code }));
        if (!response.ok) {
    ...
    async metadata() {
        const stored = await this.state.storage.get(METADATA_KEY);
    ...
    async sendGateway(socket, serverId, type, body) {
        socket.send(await signEnvelope(type, serverId, body, this.env.GATEWAY_ED25519_PRIVATE_KEY));
    }
    async sendGatewaySafely(socket, serverId, type, body, attachment) {
        try {
            await this.sendGateway(socket, serverId, type, body);
            return true;
        }
        catch {
            const a = attachment ?? socket.deserializeAttachment();
    ...
    async sendToAgent(kind, type, body) {
    ...
    safeDashboardSend(socket, payload, attachment) {
        try {
    ...
    recordDiagnostic(code, a, messageType) {
    ...
    ready(m, a) {
        const scopes = a.access?.scopes ?? [], capabilities = {};
        ...
        return {
            ...
            server: {
                ...
                paperSession: this.agents("PAPER")[0]?.deserializeAttachment()?.sessionNonce,
                paired: m.paired,
            },
            receivedAt: new Date().toISOString(),
        };
    }
    async broadcastReady(m) {
    ...
    async disconnected(socket) {
        const a = socket.deserializeAttachment();
        if (a.role !== "agent" || !a.authenticated)
            return;
        a.authenticated = false;
        socket.serializeAttachment(a);
        const m = await this.metadata();
...
function messageTypeHint(message) {
...
function protocolRejectionCode(error, authenticated) {
    if (error instanceof SyntaxError)
        return "INVALID_ENVELOPE";
    const message = error instanceof Error ? error.message : String(error);
    if (/wrong room/i.test(message))
        return "WRONG_SERVER";
    if (/protocol|agent kind/i.test(message))
    ...
    if (/initial sequence/i.test(message))
        return "INVALID_INITIAL_SEQUENCE";
    if (/challenge|candidate identity/i.test(message))
        return "CHALLENGE_REJECTED";
    if (/authentication required/i.test(message))
        return "AUTH_REQUIRED";
    if (/invalid access snapshot/i.test(message))
        return "ACCESS_SYNC_INVALID";
    if (/host authorization changed/i.test(message))
        return "HOST_AUTHORIZATION_CHANGED";
    if (/event not allowed|presence|save lease|host backups disabled|only paper reports|invalid name|invalid eventId|invalid sessionId|invalid uuid|invalid observedAt|invalid sessionStartedAt|invalid sessionEndedAt|invalid persistence state/i.test(message))
        return "INVALID_EVENT";
    if (/pairing|only paper issues|only paper approves/i.test(message))
        return "INVALID_EVENT";
    if (/envelope|timestamp|message id|missing|required|expected|invalid .*field|too large/i.test(message))
        return "INVALID_ENVELOPE";
...
export function currentAccess(m, access) {
    if (!access ||
        ...
        return false;
    return m.devices.some((d) => d.deviceId === access.deviceId &&
        d.role === access.role &&
        d.expiresAt === access.expiresAt &&
        d.issuedAt === access.issuedAt &&
        [...d.scopes].sort().join() === [...access.scopes].sort().join());
}
export function filterEvent(type, body, scopes, caps) {
    const has = (scope) => scopes.includes(scope) && caps[scope] === true;
    if (type.startsWith("telemetry."))
        return has("telemetry.view") ? body : null;
    if (type === "service.status")
        return has("server.status") ? body : null;
    if (type === "backup.progress")
        return has("backup.view") ? body : null;
    if (type === "inventory.plugins")
        return has("plugins.view") ? body : null;
    if (type === "chat.message")
        return has("chat.view") ? body : null;
    if (type === "console.lines") {
        if (!has("console.view.full") && !has("console.view.errors"))
            return null;
    ...
    if (type === "inventory.players") {
        ...
        const players = Array.isArray(body.players)
            ? body.players
                ...
                .map((p) => {
                const copy = { ...p };
                if (!has("players.address"))
                    delete copy.address;
                if (!has("players.location"))
...
function validateActionParameters(action, parameters) {
    ...
    if (action !== "players.history.list")
        return;
    ...
    if (parameters.status !== undefined &&
        ...
        throw new Error("INVALID_PARAMETERS");
    const from = optionalInstant(parameters.from), to = optionalInstant(parameters.to);
...
function validatePresenceEvent(body) {
    ...
    const observed = strictInstant(body.observedAt, "observedAt"), started = strictInstant(body.sessionStartedAt, "sessionStartedAt");
    if (started > observed + 5000)
        throw new Error("Invalid presence timestamps");
    if (!["OPEN", "QUIT", "KICK", "UNKNOWN_DISCONNECT"].includes(body.termination))
        throw new Error("Invalid presence termination");
    if (!["DISABLED", "QUEUED", "DEGRADED"].includes(body.persistenceState))
        throw new Error("Invalid persistence state");
    if (body.state === "JOINED") {
        if (body.termination !== "OPEN" ||
    ...
    if (body.termination === "OPEN")
        throw new Error("Invalid left presence event");
...
function strictInstant(value, name) {
    const text = requiredText(value, name, 40), parsed = Date.parse(text);
...
function roomFetch(request, env, serverId, values) {
...
function internalRequest(path, env, body) {
...
function identityFromHello(body, publicKey, fingerprint) {
    return {
        publicKey,
        fingerprint,
        ...
        hostPublicKey: typeof body.hostPublicKey === "string" ? body.hostPublicKey : "",
    };
}
function registrationFrom(body, lookupId) {
...
function allowedOrigins(env) {
    return new Set(env.DASHBOARD_ORIGINS.split(",")
        .map((value) => value.trim())
        .filter(Boolean));
}
function dashboardOriginAllowed(request, env) {
