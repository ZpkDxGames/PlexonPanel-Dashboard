...
export async function loadStandaloneConfig(env = process.env) {
    ...
    if (!LOOPBACK.has(host) && !allowPublicBind) {
        throw new Error("PLEXON_RELAY_HOST must be loopback unless PLEXON_RELAY_ALLOW_PUBLIC_BIND=true is set intentionally.");
    }
    const port = integer(env.PLEXON_RELAY_PORT, "PLEXON_RELAY_PORT", 8787, 1, 65535);
    const databasePath = resolve(text(env.PLEXON_RELAY_DATABASE_PATH, "PLEXON_RELAY_DATABASE_PATH", "./relay.db"));
    ...
    if (!origins.length)
        throw new Error("PLEXON_RELAY_DASHBOARD_ORIGINS is required");
    const dashboardOrigins = new Set();
    for (const originText of origins) {
        if (originText === "*")
            throw new Error("Wildcard dashboard origins are forbidden");
    ...
    const trustedProxyRaw = text(env.PLEXON_RELAY_TRUSTED_PROXY, "PLEXON_RELAY_TRUSTED_PROXY", "cloudflare-loopback");
    ...
    const config = {
        ...
        relayPrivateKey,
        relayPublicKey,
        dashboardLimit: integer(env.PLEXON_RELAY_DASHBOARD_LIMIT, "PLEXON_RELAY_DASHBOARD_LIMIT", 12, 1, 64),
        unauthenticatedLimit: integer(env.PLEXON_RELAY_UNAUTHENTICATED_LIMIT, "PLEXON_RELAY_UNAUTHENTICATED_LIMIT", 8, 1, 32),
        handshakeTimeoutMs: integer(env.PLEXON_RELAY_HANDSHAKE_TIMEOUT_MS, "PLEXON_RELAY_HANDSHAKE_TIMEOUT_MS", 15_000, 5_000, 60_000),
        gracefulShutdownMs: integer(env.PLEXON_RELAY_GRACEFUL_SHUTDOWN_MS, "PLEXON_RELAY_GRACEFUL_SHUTDOWN_MS", 10_000, 1_000, 60_000),
...
async function verifyRelayKeyPair(privateBase64, publicBase64) {
    try {
        const privateKey = await crypto.subtle.importKey("pkcs8", fromBase64(privateBase64), { name: "Ed25519" }, false, ["sign"]);
        const publicKey = await crypto.subtle.importKey("spki", fromBase64(publicBase64), { name: "Ed25519" }, false, ["verify"]);
        ...
        throw new Error(`Invalid relay Ed25519 keypair: ${error instanceof Error ? error.message : "unknown error"}`);
    }
}
function csv(value) {
    return (value ?? "")
