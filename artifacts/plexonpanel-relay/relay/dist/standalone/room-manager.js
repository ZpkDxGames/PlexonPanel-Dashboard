...
async function processHostConsole(roomObject, session, text) {
    ...
    if (session.consoleHealthy !== true)
        return;
    const hostCanView = room.metadata.hostIdentity?.capabilities["console.view.full"] === true ||
    ...
    for (const dashboard of room.dashboards.values()) {
        if (!room.currentAccess(dashboard.access))
            continue;
        const filtered = filterEvent("console.lines", body, dashboard.access.scopes, room.metadata.hostIdentity?.capabilities ?? {});
        if (!filtered)
            continue;
...
const prototype = CoreRoom.prototype;
...
prototype.agentMessage = async function (session, text) {
    let type = "";
    try {
...
prototype.dashboardMessage = async function (session, text) {
...
prototype.ready = function (session) {
