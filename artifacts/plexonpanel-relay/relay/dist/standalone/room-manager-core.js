...
import { pairingLookupId } from "../security.js";
...
export class RoomManager {
    ...
    counters;
    rooms = new Map();
    constructor(store, config, counters) {
        this.store = store;
        this.config = config;
...
export class Room {
    ...
    claiming = false;
    constructor(serverId, metadata, store, config, counters) {
    ...
    acceptsDashboard(access) {
        return this.currentAccess(access);
    }
    attachAgent(socket, kind) {
        if (this.socketCounts().unauthenticated >= this.config.unauthenticatedLimit)
            return false;
        const session = {
            socket,
            kind,
    ...
    attachDashboard(socket, access) {
        ...
        if (existing)
            existing.socket.close(4002, "Device reconnected");
        const session = {
            socket,
            access,
            rate: [],
        ...
        socket.onClose((_code, _reason) => {
            if (this.dashboards.get(access.deviceId) === session)
                this.dashboards.delete(access.deviceId);
        });
    ...
    async claimPairing(registration, name) {
        if (this.claiming ||
    ...
    closeAll() {
        ...
        for (const candidate of this.candidates)
            candidate.socket.close(1001, "Relay shutting down");
        for (const dashboard of this.dashboards.values())
            dashboard.socket.close(1001, "Relay shutting down");
    }
    async agentMessage(session, text) {
        ...
        if (body._session !== session.sessionNonce ||
        ...
        if (envelope.type === "agent.challenge_response") {
            if (session.authenticated ||
                ...
                !(await verifyChallengeProof(session.challenge, body.proof, await importAgentPublicKey(session.publicKey))) ||
                !session.candidate) {
                this.counters.authenticationFailures += 1;
                throw new Error("Challenge rejected");
            }
            if (session.kind === "PAPER")
                this.metadata.identity = session.candidate;
            else
                this.metadata.hostIdentity = session.candidate;
        ...
        for (const dashboard of this.dashboards.values()) {
            if (!this.currentAccess(dashboard.access))
                continue;
            const filtered = filterEvent(envelope.type, body, dashboard.access.scopes, identity?.capabilities ?? {});
            if (filtered)
    ...
    async dashboardMessage(session, text) {
        ...
        try {
            ...
            if (!scope || !session.access.scopes.includes(scope))
            ...
            if (transfer)
                session.transferRate = [...rate, now];
            else
                session.rate = [...rate, now];
            ...
            if (action === "players.snapshot.request") {
                if (session.snapshotRate > now - 5_000)
                    throw new Error("RATE_LIMITED");
            ...
            const ownPending = [...this.pending.values()].filter((pending) => pending.deviceId === session.access.deviceId).length;
            if (this.pending.size >= 64 || ownPending >= 32)
    ...
    agentDisconnected(session, code, reason) {
        clearTimeout(session.authTimer);
        this.candidates.delete(session);
        const wasAuthoritative = (session.kind === "PAPER" && this.paper === session) ||
            (session.kind === "HOST" && this.host === session);
        if (session.kind === "PAPER" && this.paper === session)
            this.paper = null;
        if (session.kind === "HOST" && this.host === session)
            this.host = null;
    ...
    async registerPairing(session, body) {
    ...
    resolvePairing(type, body) {
        ...
        if (type === "pairing.accepted" &&
            ...
            if (latestGeneration > grant.generation ||
                (latestGeneration === grant.generation && latestRevision > grant.revision)) {
                waiter.resolve(null);
                return;
            }
            if (latestGeneration !== grant.generation)
                this.metadata.devices = [];
            this.metadata.generation = grant.generation;
            this.metadata.revision = grant.revision;
            this.metadata.devices = this.metadata.devices.filter((device) => device.deviceId !== grant.device.deviceId);
    ...
    applyAccessSync(session, body) {
        ...
        if (Number(body.generation) < this.metadata.generation)
            return;
        if (Number(body.generation) === this.metadata.generation &&
    ...
    routeActionResult(session, body) {
        const requestId = requiredUuid(body.requestId, "requestId");
        ...
        if (!dashboard || !this.currentAccess(dashboard.access))
            return;
        this.dashboardSend(dashboard, {
            type: "server.event",
            serverId: this.serverId,
    ...
    currentAccess(access) {
        return currentAccess(this.metadata, access);
    }
    ready(session) {
        const capabilities = {};
    ...
    broadcastReady() {
        for (const dashboard of this.dashboards.values()) {
            if (this.currentAccess(dashboard.access))
                this.dashboardSend(dashboard, this.ready(dashboard));
        }
    }
    closeInvalidDashboards() {
        for (const dashboard of this.dashboards.values())
            if (!this.currentAccess(dashboard.access))
                dashboard.socket.close(4003, "Device revoked or expired");
    ...
    async sendGateway(socket, type, body) {
        const encoded = await signEnvelope(type, this.serverId, body, this.config.relayPrivateKey);
        if (!socket.send(encoded))
            throw new Error("WebSocket send failed");
    }
    dashboardSend(session, body) {
        if (!session.socket.send(JSON.stringify(body)))
            session.socket.close(1011, "Dashboard delivery failed");
    }
    persist() {
    ...
    log(code, kind, messageType, error) {
...
function protocolRejectionCode(error) {
    ...
    if (/access snapshot/i.test(message))
        return "ACCESS_SYNC_INVALID";
    return "INVALID_ENVELOPE";
}
function messageTypeHint(text) {
    try {
...
function requiredPairingCode(value) {
...
//# sourceMappingURL=room-manager-core.js.map