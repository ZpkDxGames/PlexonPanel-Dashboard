...
export class NodeWebSocket {
    ...
    static accept(request, socket, head, options = {}) {
        ...
        const peer = new NodeWebSocket(socket, Math.min(options.maxMessageBytes ?? MAX_FRAME_BYTES, MAX_FRAME_BYTES));
        if (head.length)
            peer.receive(head);
        return peer;
    }
    ...
    ping(payload = Buffer.alloc(0)) {
        if (payload.length > 125 || !this.isOpen())
            return false;
    ...
    terminate(reason = "Transport terminated") {
        if (this.closed)
            return;
        this.closed = true;
        this.socket.destroy();
    ...
    parseFrame() {
        ...
        if (rsv || !masked)
            throw new Error("Unsupported WebSocket frame flags");
        ...
        if (opcode === 0x1) {
            if (this.fragmentOpcode)
                throw new Error("Unexpected text frame during fragmentation");
            if (fin)
    ...
    emitText(payload) {
        ...
        if (!Buffer.from(text, "utf8").equals(payload))
            throw new Error("Invalid UTF-8 WebSocket text");
        this.messageHandler(text);
    }
    writeFrame(opcode, payload, allowClosed = false) {
        if ((!allowClosed && !this.isOpen()) || this.socket.destroyed)
            return false;
        let header;
...
export function websocketProtocols(request) {
