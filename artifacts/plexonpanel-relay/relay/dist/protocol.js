...
export function decodeEnvelope(text) {
    ...
    if (bodyBytes.byteLength > MAX_BODY_BYTES)
        throw new Error("Protocol body is too large");
    let body;
    try {
 ...
 */
function normalizeLegacyBody(type, body) {
    if (type !== "players.presence")
        return body;
    if (Object.prototype.hasOwnProperty.call(body, "sessionEndedAt") &&
...
export function assertFreshEnvelope(envelope, now = Date.now()) {
...
export async function importAgentPublicKey(publicKeyBase64) {
    if (publicKeyBase64.length < 32 || publicKeyBase64.length > 512)
...
export async function verifyEnvelope(envelope, publicKey) {
    try {
        return crypto.subtle.verify({ name: "Ed25519" }, publicKey, fromBase64Url(envelope.signature), new TextEncoder().encode(signable(envelope)));
    }
    catch {
        return false;
    }
}
export async function verifyChallengeProof(nonce, proof, publicKey) {
    if (typeof proof !== "string" || proof.length > 256)
        return false;
    try {
        return crypto.subtle.verify({ name: "Ed25519" }, publicKey, fromBase64Url(proof), new TextEncoder().encode(`challenge:${nonce}`));
    }
    catch {
        return false;
    }
}
export async function signEnvelope(type, serverId, body, privateKeyBase64) {
    ...
    const signature = await crypto.subtle.sign({ name: "Ed25519" }, key, new TextEncoder().encode(signable(envelope)));
...
function validateEnvelope(envelope) {
    ...
    if (typeof envelope.timestamp !== "string" || envelope.timestamp.length > 64)
        throw new Error("Invalid timestamp");
    if (typeof envelope.body !== "string" ||
        envelope.body.length < 2 ||
        envelope.body.length > MAX_BODY_BYTES * 2 ||
        !/^[A-Za-z0-9_-]+$/.test(envelope.body)) {
