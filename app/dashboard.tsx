  ...
  return (
    <main className="access-screen">
      <section className="access-brand-panel">
        ...
        <p className="access-footnote">Vercel stores no server data. This device keeps only its scoped credential and bounded cache in IndexedDB.</p>
      </section>
      <section className="access-form-panel">
        <div className="access-card">
          ...
          {unavailable ? (
            <div className="access-unavailable" role="alert"><strong>Browser workspace unavailable</strong><span>Allow local site storage, then try again.</span><button className="secondary-button" onClick={onRetry}>Retry</button></div>
...
export default function Dashboard() {
  ...
  const refreshSession = useCallback(async () => {
    ...
    try {
      ...
      if (!session.authenticated || !session.serverId) {
        setWorkspace(null);
        setAccess("unpaired");
        return;
      }
      setWorkspace(session.workspace
        ? applyConnectionStatus(session.workspace, "offline")
        : transformRelayState({
            connectionStatus: "offline",
            server: { serverId: session.serverId, paired: true },
          }));
      setAccess("ready");
    } catch {
  ...
  useEffect(() => {
    if (!workspace || access !== "ready") return;
    const timer = window.setTimeout(() => void cacheDashboardWorkspace(workspace), 500);
    return () => window.clearTimeout(timer);
  }, [workspace, access]);
      ...
      try {
        ...
        socket.onmessage = (event) => {
          ...
          if (message.type === "dashboard.ready") {
            setWorkspace((current) => applyRelayReady(current, record(message.server), message.connectionStatus));
          } else if (message.type === "server.event" && typeof message.eventType === "string") {
            setWorkspace((current) => current ? applyRelayEvent(current, message.eventType as string, record(message.body)) : current);
        ...
        if (cancelled) return;
        if (failure instanceof DashboardRequestError && failure.status === 401) {
          void logoutDashboard().finally(() => { setWorkspace(null); setAccess("unpaired"); });
          return;
        }
    ...
    <div className="dashboard-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          <div className={`connection-card ${live ? "connected" : ""}`}><span className="status-beacon"><i /></span><div><strong>Relay {transportLabel.toLowerCase()}</strong><span>{overview.server.lastSeen}</span></div></div>
          <button className="profile-button" onClick={() => void signOut()} aria-label="Forget this browser credential"><span className="avatar">ZD</span><span className="profile-copy"><strong>Administrator</strong><small>Forget this browser</small></span><span className="more-dots">•••</span></button>
        </div>
      </aside>
